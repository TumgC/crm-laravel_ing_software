...
<div class="bg-white p-6 rounded-xl shadow-sm border mt-6">
    ...
    <?php if($ticket->satisfactionSurvey): ?>
        <div class="rounded-lg border border-green-200 bg-green-50 p-4 text-green-800">
            <p><strong>Calificación:</strong> <?php echo e($ticket->satisfactionSurvey->rating); ?>/5</p>

            <?php if($ticket->satisfactionSurvey->comment): ?>
                <p class="mt-2"><strong>Comentario:</strong> <?php echo e($ticket->satisfactionSurvey->comment); ?></p>
            <?php endif; ?>

            <p class="text-sm mt-2">
                Registrada el:
                <?php echo e($ticket->satisfactionSurvey->answered_at->format('d/m/Y h:i A')); ?>

            </p>
        </div>
    <?php elseif(in_array($ticket->status, ['Resuelto', 'Cerrado'])): ?>
        <form method="POST" action="<?php echo e(route('tickets.satisfaction.store', $ticket)); ?>" class="space-y-4">
            <?php echo csrf_field(); ?>
